...
async def onb_demo(callback: CallbackQuery, state: FSMContext):
    demo_image_path = "src/bot/data/demo_image_1.png"
    ...
    🌿 Растение: Ficus elastica (Фикус каучуконосный, сорт с пестрыми листьями)
    📊 Health Score: 😕 5/10 (Повреждены и частично высохли листья)
    Вердикт:
    На листе заметны крупные коричневые и сухие пятна, что говорит о проблемах с поливом или ожогах от солнца. Возможно, растение пострадало от переувлажнения, резких перепадов температур или слишком яркого света. Рекомендую убрать фикус с прямого солнца, поливать только после подсыхания верхнего слоя почвы и проверить, чтобы в горшке был хороший дренаж.
    """
    ...
    demo_image_path = "src/bot/data/demo_image_2.png"
    await callback.message.answer_photo(
        photo=FSInputFile(demo_image_path),
        caption=text
    )
